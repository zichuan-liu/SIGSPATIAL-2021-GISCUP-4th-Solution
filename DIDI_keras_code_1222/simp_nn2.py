...
state_fea =  ['state_num_0','state_num_1','state_num_2','state_num_3', 'state_sum_time_0', 'state_sum_time_1', 'state_sum_time_2', 'state_sum_time_3']#
useless_fea = ['order_id','ata', 'date', 'd_s', 'cross_num_ata_mean']+x_graph_fea+state_fea
data, sparse_features, dense_features = recognize_feature(data, categorical_columns, lable=useless_fea)


################################
#import re
#log_features = [x for x in dense_features if not bool(re.search(r'\d', x))]
#print(log_features)#

#for i,col in tqdm(enumerate(log_features)):
#    data[col+"_log"] = np.log(data[col]+1)
#    dense_features.append(col+"_log")
...
folds = KFold(n_splits=n_splits, shuffle=shuffle, random_state=N_SEED)
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __init__(self, df_train_head, train_rnn, train_rnn_crosses, batch_size=n_batch, shuffle=shuffle, is_train = True):
...
from warmup import WarmUpLearningRateScheduler
...
for fold_, (trn_idx, val_idx) in enumerate(folds.split(df_train_head.values)):
    ...
    reduce_lr = ReduceLROnPlateau(monitor='val_mape', factor=0.4, patience=1, min_lr=min_LR, mode='min')
    #warm_up_lr = WarmUpLearningRateScheduler(n_batch, init_lr=LR)
    es = tf.keras.callbacks.EarlyStopping(patience=patience, monitor='val_mape', mode='min',
              restore_best_weights=True)
    #cosine_decay = tf.keras.experimental.CosineDecay(
    #            initial_learning_rate=LR, decay_steps=100)
    model.compile(
    ...
    model.save_weights("./savemodel/wdr_model2_fold_"+str(fold_)+".h5")
...
submission.to_csv('subs/simple_wdr_lk_sub2.csv',index=False)
