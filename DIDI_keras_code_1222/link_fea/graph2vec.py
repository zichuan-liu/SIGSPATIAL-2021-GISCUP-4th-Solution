...
def main(args):
    ...
    """
    filenames = os.listdir(args.input_path+"_"+args.feature_type+"/")
    filenames.sort(key=lambda x: int(x[-3:]))
    #filenames = ['g_20200901']
    for i in filenames:  
      print(i)
      
      graphs = glob.glob(args.input_path+"_"+args.feature_type+"/" + i +'/' + "*.json")
      ...
      save_embedding(args.output_path+'_'+args.feature_type+"/"+i+'.csv', model, graphs, args.dimensions)
