...
if is_w2v:
    ...
    if not is_save_w2v:
        ...
        for file in tqdm(filenames):
            try:
                ...
                if file[9:14] == 'links':
                    ...
                    lkid_list1 = train_rnn1[:, :, 3].astype(int).tolist()  # his id
        ...
        lkid_list_test = df_test_rnn[:, :, 3].astype(int).tolist()  # his id
        ...
        model = models.Word2Vec(lkid_list, workers=8, min_count=0, window=2, size=8)
        model.save('model_all_day_stage_w2v')  #
        model = models.Word2Vec.load('model_all_day_stage_w2v')  #
        ...
        lkid_vecs.columns = ["w2v_all_day_stage_" + str(x) for x in range(8)]

        lkid_vecs.to_pickle(r'../data/w2v_fea/all_stage_vecs.pkl')
